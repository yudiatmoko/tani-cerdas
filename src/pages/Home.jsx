...
export default function Home() {
  ...
  const handleNext = () => {
    setCurrentIndex(
      (prevIndex) => (prevIndex < testimonials.length - 2 ? prevIndex + 1 : 0) // Kembali ke awal jika sudah di akhir
    );
  };

  const handlePrev = () => {
    setCurrentIndex(
      (prevIndex) => (prevIndex > 0 ? prevIndex - 1 : testimonials.length - 2) // Jika di awal, pergi ke akhir
    );
  };
  ...
  return (
    <>
        ...
        <div className="flex flex-col-reverse lg:flex-row bg-primary-600 w-full h-auto mx-auto py-8 lg:py-20 justify-center items-center gap-4 lg:gap-20">
          <div className="flex px-8 py-8 lg:px-16 justify-center items-start flex-col gap-12">
            <h1 className="text-white text-[48px] lg:text-[54px] text-balance font-bold font-['Roboto'] leading-[60px]">
              Optimalkan Pertanian Anda dengan Bimbingan Profesional di Tani
              Cerdas
            </h1>
            <p className="font-bold text-lg text-white text-pretty">
              ...
              pertanian dan meningkatkan hasil panennya.
            </p>
            <CustomButton
              onClick={() =>
                document
      ...
      <div>
        <div
          id="product"
          className="flex flex-col bg-primary-50 w-full h-auto mx-auto p-8 lg:p-20 justify-start items-center gap-8 lg:gap-16"
        >
